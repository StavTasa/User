...
def test_create_users(test_client):
    ...
    response = test_client.post('/users', json=data_1)
    assert response.status_code == 201
    print_user_to_file(test_client, 10)
    response = test_client.post('/users', json=data_2)
    assert response.status_code == 201
...
def test_get_user(test_client):
    response = test_client.get('/users/10')
...
def test_update_user(test_client):
    ...
    response = test_client.put('/users/10', json=data)
    assert response.status_code == 200
    print_user_to_file(test_client, 10)
...
def test_create_existing_user(test_client):
    ...
    response = test_client.put('/users/11', json=data)
